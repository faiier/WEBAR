...
const quizData = [
  ...
  function loadModel(modelPath) {
    ...
    modelEntity.setAttribute('position', '0 0.2 -1.5');
    ...
    // จัดตำแหน่งโมเดลเมื่อโหลดเสร็จ
    modelEntity.addEventListener('model-loaded', () => {
      centerModel();
    });
  }
  
  // จัดตำแหน่งโมเดลให้อยู่กลางจอ
  function centerModel() {
    if (!modelEntity || !modelEntity.object3D) return;
    
    // คำนวณขนาดโมเดล
    const box = new THREE.Box3().setFromObject(modelEntity.object3D.children[0]);
    ...
    modelEntity.setAttribute('data-base-scale', scale);
  }
